...
import { isExpectedDay, habitColor, groupHabitsByTimeOfDay } from '../../lib/habitUtils'
...
export default function TodayView({ habits, today, todayStr, logsByHabit, sims, onToggleLog }) {
  ...
  const groups = groupHabitsByTimeOfDay(todaysHabits)

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {groups.map(group => (
        <div key={group.key}>
          <p className="mono" style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 8 }}>{group.label}</p>
