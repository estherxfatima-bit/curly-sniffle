...
import { Plus, Trash2, Sparkles, PoundSterling } from 'lucide-react'
...
export default function FinancePage() {
  ...
  async function load() {
    setLoading(true)
    const [incRes, fixRes, varRes] = await Promise.all([
  ...
  const spendPct      = totalIncome > 0 ? Math.min(100, Math.round(((totalFixed + totalVariable) / totalIncome) * 100)) : 0
    ...
    <div>
      ...
      <div className="page-header header-finance mb-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1>Finance</h1>
            <p>Monthly overview — income, expenses, and tax pot</p>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 16, marginBottom: 28 }}>
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          <ArcRing value={totalIncome} max={totalIncome || 1} size={90} color="var(--finance)" label={`£${totalIncome.toFixed(0)}`} sublabel="Income/mo" />
        </div>
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          <ArcRing value={totalFixed + totalVariable} max={totalIncome || 1} size={90} color="var(--personal)" label={`£${(totalFixed + totalVariable).toFixed(0)}`} sublabel="Spend/mo" />
          <p className="mono" style={{ fontSize: 10, color: 'var(--text-3)' }}>{spendPct}% of income</p>
        </div>
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          <ArcRing value={taxPot} max={totalIncome || 1} size={90} color="var(--creative)" label={`£${taxPot.toFixed(0)}`} sublabel="Tax pot" />
          <p className="mono" style={{ fontSize: 10, color: 'var(--text-3)' }}>25% self-emp</p>
        </div>
        <div className="card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
          <ArcRing value={Math.max(0, takeHome)} max={totalIncome || 1} size={90} color={takeHome >= 0 ? 'var(--finance)' : 'var(--danger)'} label={`£${takeHome.toFixed(0)}`} sublabel="Take-home" />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        {/* Fixed expenses */}
        <div className="card">
          <h3 style={{ fontSize: '0.9rem', marginBottom: 14 }}>Fixed expenses <span className="mono" style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 400 }}>monthly</span></h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
            {fixed.map(i => (
              <div key={i.id} className="flex items-center justify-between gap-2">
                <div>
      ...
      <div className="card mt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button className="btn btn-sm btn-finance" style={{ color: '#fff' }} onClick={runAISummary} disabled={aiLoading || income.length === 0}>
            <Sparkles size={12} />
